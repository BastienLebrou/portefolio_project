...
vegevigie interface  forets_vegevigie.gpkg  bati.gpkg \
  ...
  --config <...>/scrutech/plugins/qgis/scrutech/config/default.yaml
```
...
1. **Bug pré-existant dans la copie scrutech (non introduit par ce travail)** :
  ...
  `scrutech/plugins/qgis/config/` au lieu de `scrutech/plugins/qgis/scrutech/config/`. Artefact de recopie
