...
1. Build the zip: `python scrutech/plugins/qgis/package.py` writes `scrutech/plugins/qgis/dist/scrutech.zip`
...
For development, link `scrutech/plugins/qgis/scrutech` into the QGIS profile's
`python/plugins/` folder and run `package.py` once; the tools then use
`scrutech/packages/vegevigie/.venv`.
