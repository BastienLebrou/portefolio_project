...
| Commande | Effet |
|---|---|
| `python outil/run_pipeline.py` | Tout : (re)génère les données puis exécute le pipeline |
| `python outil/run_pipeline.py --no-generate` | Réutilise les données déjà présentes dans `data/raw/` |
| `python outil/run_pipeline.py --quiet` | Moins de logs |
...
Modifie, relance `python outil/run_pipeline.py` : toutes les couches sont régénérées.
...
`data/sources_reelles/`, on lance `python outil/real_data_adapter.py`, puis
`python outil/run_pipeline.py --no-generate`.
...
| 0 parcelle éligible | Seuils trop stricts dans `outil/config.py`, ou données d'entrée vides |
