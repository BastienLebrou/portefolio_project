...
```bash
python real_data_adapter.py           # convertit -> data/raw/*.parquet
python run_pipeline.py --no-generate  # exécute le pipeline sur le réel
```
...
```bash
python download_fiber_data.py          # fibre ARCEP -> Data_alba/fibre.parquet
python download_classified_woodlands.py # EBC (GPU) -> Data_alba/ebc.parquet
```

- **`download_fiber_data.py`** : « Ma connexion internet » (data.arcep.fr). Joint la
...
- **`download_classified_woodlands.py`** : Espaces Boisés Classés via le WFS du Géoportail de
...
- **Colonnes manquantes** : si une source n'a pas la colonne attendue (ex. pas de
  ...
  sera alors peu discriminant. Adapte les noms dans `real_data_adapter.py`
