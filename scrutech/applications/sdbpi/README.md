...
├── config.py             # zone, buffer, usages cibles, source SIRENE et chemins
├── network.py            # session HTTP robuste et erreurs claires
├── sources.py            # acquisition et cache des données géographiques
├── processing.py         # fonctions pures GeoPandas de filtrage et synthèse
├── run_vacancy_analysis.py # orchestration + CLI
...
python run_vacancy_analysis.py --insee 01053

# Buffer plus large (recommandé en zone industrielle, voir plus bas)
python run_vacancy_analysis.py --insee 01053 --buffer 30

# Emprise bbox WGS84 (minx,miny,maxx,maxy)
python run_vacancy_analysis.py --bbox 5.21,46.19,5.25,46.22

# Restreindre les usages cibles
python run_vacancy_analysis.py --insee 01053 --usages "Industriel"
...
python run_vacancy_analysis.py --emprise emprise_etude.parquet --source grandlyon

# Source SIRENE = fichier départemental pré-géocodé (passage à l'échelle)
python run_vacancy_analysis.py --insee 69123 --source geo_file --geo-file C:\data\sirene_geo_69.parquet

# Ignorer le cache
python run_vacancy_analysis.py --insee 01053 --no-cache
```
